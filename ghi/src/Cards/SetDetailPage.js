...
function SetDetailPage(props) {
    ...
    const getBoosterSet = async() => {
        const boosterSetData = boosterSets.find(boosterSet => boosterSet.id === card_set_id)
        if (boosterSetData) {
            ...
            for (let card_number of boosterSetData.mv) {
                maxVariablesList.push(cards.find(card => card.card_number === card_number))
            }
            setMaxVariables(maxVariablesList)
            for (let card_number of boosterSetData.normals) {
                normalsList.push(cards.find(card => card.card_number === card_number))
            }
            setNormals(normalsList)
            for (let card_number of boosterSetData.rares) {
                raresList.push(cards.find(card => card.card_number === card_number))
            }
            setRares(raresList)
            for (let card_number of boosterSetData.super_rares) {
                superRaresList.push(cards.find(card => card.card_number === card_number))
            }
            setSuperRares(superRaresList)
            for (let card_number of boosterSetData.ultra_rares) {
                ultraRaresList.push(cards.find(card => card.card_number === card_number))
            }
